...
void CommTiled::setup()
{
  ...
  // set boxlo/boxhi subboxlo/subboxhi
  if (triclinic == 0) {
    prd = domain->prd;
    boxlo = domain->boxlo;
    boxhi = domain->boxhi;
  ...
  if (mode == Comm::MULTI) {
    ...
    for (i = 0; i < ncollections; i++) {
      if (cutusermulti) {
                              //per collection user ghost cutoff (multi)
      ...
      //cutghostmulti: cutghost on a per-collection basis
  ...
  if (mode == Comm::MULTIOLD) {
    ...
    for (i = 1; i <= ntypes; i++) {
      double tmp = 0.0;
      // cutghostmultiold: cutghost on a per-type basis
